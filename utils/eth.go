...
func CheckIfExpired(payment *model.Payment, balance *big.Int) bool {
	if payment.CreatedAt.Add(15 * time.Minute).Before(time.Now()) {
		if balance == nil {
			paid, _ := IsPaidOnChain(payment)
			return paid
		} else {
			return payment.IsPaid(balance)
		}
	}
...
func Forward(client *ethclient.Client, payment *model.Payment) *types.Transaction {
	...
	chainGateEarnings := GetChaingateEarnings(&payment.CurrentPaymentState.PayAmount.Int)
